...
Windows에서 COM 기반 Hancom 검증 헬퍼까지 쓰려면 `pywin32`도 설치합니다.
...
| `discover_hancom_executable()` | `Path \| None` | 설치된 Hancom 실행 파일 탐색 |
| `hancom_open_validation_errors(executable_path=None, timeout_seconds=15)` | `list[str]` | 임시 파일을 Hancom으로 열어 보고 실패를 반환 |
| `open_in_hancom(executable_path=None, timeout_seconds=15)` | `None` | Hancom으로 임시 파일을 열고 실패 시 예외 발생 |
...
Windows에서 Hancom이 설치되어 있다면 `open_in_hancom()`이나 `hancom_open_validation_errors()`도 사용할 수 있습니다. 자동 탐색으로 충분하지 않을 때만 `HWPX_HANCOM_EXE`를 지정하면 됩니다.
...
- Hancom 모델 검증은 Windows와 로컬 Hancom 설치가 필요합니다.
